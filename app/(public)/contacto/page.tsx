...
export default async function ContactoPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      ...
      {/* Botones CTA arriba en móvil */}
      <div className="flex flex-col sm:flex-row gap-3 mb-10 sm:mb-0 sm:hidden">
        {config.whatsapp_number && (
          <a
            href={buildWhatsAppContactUrl(config.whatsapp_number)}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full px-6 py-4 bg-[#25D366] text-white text-sm font-semibold rounded-xl hover:bg-[#20ba59] active:bg-[#1da851] transition-colors shadow-sm"
          >
        ...
        {config.instagram_url && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full px-6 py-4 bg-gradient-to-r from-purple-600 to-pink-500 text-white text-sm font-semibold rounded-xl hover:opacity-90 active:opacity-80 transition-opacity shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12">
        <div className="flex flex-col gap-5">
          ...
          {/* Botones CTA — solo en desktop */}
          <div className="hidden sm:flex flex-col sm:flex-row gap-3 mt-2">
            {config.whatsapp_number && (
              <a
                href={buildWhatsAppContactUrl(config.whatsapp_number)}
                target="_blank"
